...
const CreatePG = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (state.photo) {
        ...
        imgUrl = uploadRes.data.url; // assumed backend returns { url: '...' }
      }
      ...
      await axios.post("http://localhost:8000/api/pg", listingData);

      alert("PG listing created!");
    } catch (error) {
      console.error("Failed to create PG listing:", error);
      alert("Error: Could not create PG listing");
    }
  }
  ...
  return (
    <div>
      ...
      <input type="file" accept='image/*' onChange={(e) => dispatch({ type: "photo", payload: e.target.files[0] })} />
      <select name="gender" onChange={(e) => dispatch({ type: "gender", payload: e.target.value })}>
